...
extern "C"{
  ...
  void ml5_0_sloopmatrix_thres_(double*,complex<double>(*),double&,double[2][2],int&);
}
...
double TotPhaseSpace(double *x, size_t dim, void *params)
{
  ...
  complex<double> *xx2;
  xx2=new complex<double>[4*Len*Len];    
  int ret_code=0;
  double prec_ask=-1;
  ...
  if(mel==0)
    {
      ...
      tmp=TraceBornPhase(ppart,chan,M2)*2.;
  ...
  else if(mel==1) //Diff
    {
      ...
      for(int l=0; l<4; l++)
	{
	  tmp=ppart[1+(npart+2)*l];
	  ppart[1+(npart+2)*l]=ppart[0+(npart+2)*l];
	  ppart[0+(npart+2)*l]=tmp;

	  tmp=ppar[1*4+l];
	  ppar[1*4+l]=ppar[0*4+l];
	  ppar[0*4+l]=tmp;
	}
      
      ml5_0_sloopmatrix_thres_(ppar,xx2,prec_ask,prec_f,ret_code);

      res+=std::imag(TracePhase(ppart,chan,M2,xx2,x[d])*ColinearPhase(x[d],chan,M2)*MellinPDFPhase(x[d],chan,M2)*GlobalPhase(x[d],sc,M2)*jac);

      res-=std::imag((TraceExpPhase(ppart,chan,M2,xx2,x[d])+(1.+ColinearExpPhase(x[d],chan,M2))*TraceBornPhase(ppart,chan,M2))*MellinPDFPhase(x[d],chan,M2)*GlobalPhase(x[d],sc,M2)*jac);
